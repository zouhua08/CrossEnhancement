...
def AttnNet_1_2(ref_input, illu_input, is_training=True):
    ...
    output_1 = AttnNet_1(ref_input)
    output_2 = AttnNet_2(illu_input)

    output_1_2 = tf.multiply(output_1, output_2)    
...
def Structure_Generator(input1, input2, input3):
    ...
    x0 = tf.layers.conv2d(inp, 64, 7, 1, padding='same', activation=tf.nn.relu)

    conv1 = tf.layers.conv2d(x0, 64, 4, 2, padding='same', activation=tf.nn.relu)
    x1 = tf.layers.conv2d(conv1, 64, 5, 1, padding='same', activation=tf.nn.relu)

    conv3 = tf.layers.conv2d(x1, 64, 4, 2, padding='same', activation=tf.nn.relu)
    x2 = tf.layers.conv2d(conv3, 64, 5, 1, padding='same', activation=tf.nn.relu)

    x3 = tf.layers.conv2d(x2, 64, 4, 2, padding='same', activation=tf.nn.relu)

    u11 = RBK(x3, 64)
    u11 = tf.image.resize_nearest_neighbor(u11, (tf.shape(u11)[1]*2, tf.shape(u11)[2]*2))
    u12 = tf.layers.conv2d(x2, 64, 5, 1, padding='same', activation=tf.nn.relu)
    u1 = u11 + u12

    u21 = RBK(u1, 64)
    u21 = tf.image.resize_nearest_neighbor(u21, (tf.shape(u21)[1]*2, tf.shape(u21)[2]*2))
    u22 = tf.layers.conv2d(x1, 64, 5, 1, padding='same', activation=tf.nn.relu)
    u2 = u21 + u22

    u31 = RBK(u2, 64)
    u31 = tf.image.resize_nearest_neighbor(u31, (tf.shape(u31)[1]*2, tf.shape(u31)[2]*2))
    u32 = tf.layers.conv2d(x0, 64, 5, 1, padding='same', activation=tf.nn.relu)
    u3 = u31 + u32
...
def Texture_Generator(input1, input2, input3, output_AF):

    dim = 64

    inp = tf.concat([input1, input2, input3, output_AF], axis=3)

    x11 = tf.layers.conv2d(inp, dim//2, 7, 1, padding='same', activation=tf.nn.relu)
    x12 = tf.layers.conv2d(x11, dim, 4, 2, padding='same', activation=tf.nn.relu)
    x13 = tf.layers.conv2d(x12, dim, 5, 1, padding='same', activation=tf.nn.relu)

    x21 = tf.layers.conv2d(x13, 2*dim, 4, 2, padding='same', activation=tf.nn.relu)
    x22 = tf.layers.conv2d(x21, 2*dim, 5, 1, padding='same', activation=tf.nn.relu)

    x31 = tf.layers.conv2d(x22, 4*dim, 4, 2, padding='same', activation=tf.nn.relu)
    x32 = tf.layers.conv2d(x31, 4*dim, 5, 1, padding='same', activation=tf.nn.relu)

    x41 = tf.layers.conv2d(x32, 8*dim, 4, 2, padding='same', activation=tf.nn.relu)
    x42 = tf.layers.conv2d(x41, 8*dim, 5, 1, padding='same', activation=tf.nn.relu)

    dim = 8*dim

    y11 = RBK(x42, dim, padding='same', activation=tf.nn.relu)
    y12 = tf.layers.conv2d_transpose(y11, dim//2, 6, 2, activation=tf.nn.relu)
    f1 = tf.concat([y12, x32], axis=3)

    y21 = tf.layers.conv2d(f1, dim//2, 5, 1, padding='same', activation=tf.nn.relu)
    y22 = tf.layers.conv2d_transpose(y21, dim//4, 6, 2, activation=tf.nn.relu)
    f2 = tf.concat([y22, x22], axis=3)

    y31 = tf.layers.conv2d(f2, dim//8, 5, 1, padding='same', activation=tf.nn.relu)
    y32 = tf.layers.conv2d(y31, 3, 3, 1, padding=tf.nn.relu, activation=None)

    return y32


def Appearance_Flow(input1, input2, input3):
    ...
    x12 = tf.layers.conv2d(x11, dim, 4, 2, padding='same', activation=tf.nn.relu)
    x13 = tf.layers.conv2d(x12, dim, 5, 1, padding='same', activation=tf.nn.relu)
    x14 = tf.layers.conv2d(x13, 2*dim, 4, 2, 1, padding='same', activation=tf.nn.relu)
    x15 = tf.layers.conv2d(x14, 2*dim, 5, 1, padding='same', activation=tf.nn.relu)

    x21 = tf.layers.conv2d(x15, 4*dim, 5, 1, padding='same', activation=tf.nn.relu)
    x22 = tf.layers.conv2d(x21, 8*dim, 4, 2, padding='same', activation=tf.nn.relu)

    dim = 8*dim

    y11 = RBK(x22, dim)
    y12 = tf.image.resize_nearest_neighbor(y11, (tf.shape(y11)[1]*2, tf.shape(y11)[2]*2))
    y13 = tf.layers.conv2d(y12, dim//2, 5, 1, padding='same', activation=tf.nn.relu)

    y21 = tf.layers.conv2d(y13, dim//2, 5, 1, padding='same', activation=tf.nn.relu)
    y22 = RBK(y21, dim//4)
    y23 = tf.layers.conv2d(y22, dim//8, 5, 1, padding='same', activation=tf.nn.relu)
    y24 = tf.layers.conv2d(y23, 3, 5, 1, padding='same', activation=tf.nn.relu)
...
class lowlight_enhance(object):
    def __init__(self, sess):
        ...
        self.output_1_2 = AttnNet_1_2(self.input_ref, self.input_illu, self.is_training)
        ...
        self.output = Texture_Generator(self.output0, self.output_SG, self.output_3, self.output_AF)
        ...
        self.loss_AF = tf.reduce_mean(tf.abs(self.output_AF - self.input_high))
        self.loss_SG = tf.reduce_mean(tf.abs(self.output_SG - self.input_high))

        #self.output_dis_real = Discriminator(self.input_high)
        #self.output_dis_fake = Discriminator(self.output_gen)
        ...
        self.loss = self.loss_gen + 0.5*(self.loss_AF + self.loss_SG)
